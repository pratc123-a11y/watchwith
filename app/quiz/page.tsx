...
const FILMS = [
  ...
  { id: 238, title: 'The Godfather', year: '1972', poster: '/3bhkrj58Vtu7enYsLegHnDmni7Z.jpg' },
  ...
  { id: 19404, title: 'Dilwale Dulhania Le Jayenge', year: '1995', poster: '/2CAL2433ZeIihfX1Hb2139CX0pW.jpg' },
...
export default function Quiz() {
  ...
  if (done) {
    const loved = FILMS.filter(f => votes[f.id] === 'loved')
    const meh = FILMS.filter(f => votes[f.id] === 'meh')
    const no = FILMS.filter(f => votes[f.id] === 'no')
    return (
      <main className="min-h-screen p-8 max-w-md mx-auto">
        ...
        {[{ label: '❤️ Loved', films: loved }, { label: '😐 Meh', films: meh }, { label: '👎 No', films: no }].map(group => (
          <div key={group.label} className="mb-6">
            <p className="font-medium mb-3">{group.label} ({group.films.length})</p>
            <div className="flex flex-wrap gap-2">
              {group.films.map(f => (
                <span key={f.id} className="text-sm bg-gray-100 rounded-full px-3 py-1">{f.title}</span>
              ))}
            </div>
          </div>
        ))}
          ...
          <div key={film.id} className="rounded-xl overflow-hidden border border-gray-100">
            ...
            <div className="p-2">
              ...
              <div className="flex gap-1">
                {(['loved', 'meh', 'no'] as Vote[]).map(v => (
                  <button
                    key={v}
                    onClick={() => vote(film.id, v)}
                    className={`flex-1 text-xs py-1 rounded-lg border transition-all ${
                      votes[film.id] === v
                        ? v === 'loved' ? 'bg-green-100 border-green-300 text-green-700'
                        : v === 'meh' ? 'bg-yellow-100 border-yellow-300 text-yellow-700'
