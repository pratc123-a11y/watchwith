  ...
  return (
    <main className="min-h-screen p-6 max-w-md mx-auto pb-16">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-medium text-white">Watch later</h1>
        <span className="text-gray-500 text-sm ml-auto">{films.length} films</span>
      </div>
      {films.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-gray-400 mb-2">Nothing here yet</p>
          <p className="text-gray-600 text-sm">Mark films as Watch Later when rating in a session</p>
        </div>
      ) : (
        <div className="grid grid-cols-3 gap-3">
          {films.map(film => (
            <div key={film.film_id}>
              ...
              <p className="text-xs text-blue-400">🕐 Watch later</p>
            </div>
          ))}
        </div>
      )}
